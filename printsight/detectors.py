...
Each detector takes a grayscale image (numpy.ndarray) and returns:
    ...
    - details: dict with per-defect metrics
"""
...
def detect_stringing(img_gray: np.ndarray) -> tuple[float, dict]:
    ...
    if lines is not None:
        for line in lines:
            ...
            # Exclude near-horizontal lines (likely layer lines, not stringing)
    ...
    # Normalize score based on severity relative to image area
...
def detect_layer_quality(img_gray: np.ndarray) -> tuple[float, dict]:
    """Detect layer quality issues (inconsistent extrusion, Z-banding).

    Algorithm:
        1. Compute horizontal edges (Sobel X) — layer boundaries
        2. Project gradient magnitude onto vertical axis
        3. Analyze regularity of layer boundaries via FFT
    ...
    # Vertical projection: sum each row's gradient magnitude
    # This gives peaks at layer boundaries
    ...
    # Use difference of means or simple peak detection
    peaks = []
    window = max(5, h // 50)  # adaptive window based on image height
    ...
    # Analyze peak spacing regularity
    if len(peaks) > 3:
        ...
        peak_std = 0
        regularity = 0.5  # neutral — not enough data

    # FFT analysis for periodic patterns (good layers = strong fundamental)
    fft = np.fft.rfft(row_profile)
    fft_mag = np.abs(fft[1:])  # skip DC
    if len(fft_mag) > 0:
        dominant_idx = np.argmax(fft_mag) + 1  # +1 because we skipped DC
        dominant_freq = dominant_idx / h
        # Energy concentration — good layers have energy concentrated
        total_energy = np.sum(fft_mag)
        if total_energy > 0:
            # Ratio of dominant peak to total energy
            energy_ratio = fft_mag[dominant_idx - 1] / total_energy
        else:
            energy_ratio = 0
    else:
        dominant_freq = 0
    ...
    # Combine regularity and FFT energy for final score
...
def detect_warping(img_gray: np.ndarray) -> tuple[float, dict]:
    """Detect warping (corners lifting from build plate).

    Algorithm:
        1. Find the print region via contour detection
        2. Find the bottom edge of the print
        3. Measure curvature/deviation from straight line
    ...
    # Threshold to isolate print
    _, thresh = cv2.threshold(img_gray, 0, 255,
                              cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    # Assume print is darker than background for typical 3D prints
    # (or lighter — handle both)
    white_pixels = np.sum(thresh == 255)
    total_pixels = h * w
    if white_pixels > total_pixels * 0.5:
        # Invert: print is dark, background is light
    ...
    # Get bounding rect to find bottom portion
    x, y, bw, bh = cv2.boundingRect(main_contour)

    # Find the bottom edge points (lower 20% of bounding box)
    ...
    if len(bottom_points) < 5:
        return 0.5, {"bottom_deviation": 0, "lifted_corners": 0,
                     "error": "not enough bottom points"}

    bottom_points = np.array(bottom_points, dtype=np.float32)

    # Fit a line to bottom points
    [vx, vy, cx, cy] = cv2.fitLine(bottom_points, cv2.DIST_L2, 0, 0.01, 0.01)

    # Calculate perpendicular distance from each point to the line
    # Line: (p - center) × direction = 0
    # Distance = |(p - center) × direction| / |direction|
    ...
    # Warping score: max deviation relative to bounding box height
    deviation_ratio = max_deviation / bh if bh > 0 else 0
    score = max(0.0, 1.0 - deviation_ratio * 10)

    # Count lifted corners: points with high deviation in the bottom 10% of x-range
    ...
    for i, pt in enumerate(bottom_points):
        if deviations[i] > avg_deviation * 1.5:
            # Check if it's at a corner (within 10% of x-extents)
